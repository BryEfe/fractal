...
function Summary({ initiative, user, setLike, id }) {
 ...
 return (
  <div>
   ...
   <div className="initiative-description-likes"> <p>{initiative.description}</p> <h2>{`${initiative.followers.length} ${initiative.followers.length > 1 ? "personas apoyan " : "persona apoya "} este proyecto.`}</h2></div>
  </div>
 )
}
